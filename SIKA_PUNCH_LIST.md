...
- [ ] **5. Parlay combined probability assumes independence even for same-game / same-team legs** `[claude]` · effort: **M**
  ...
  - **Why it matters:** A 3-leg parlay built from "LeBron over points + LeBron over rebounds + Lakers ML" has heavily correlated legs. Multiplying as if independent overstates the joint probability → overstates edge → systematic over-recommendation of correlated parlays.
  - **Fix:** Replace the strict-product combiner with a correlation-aware combiner. Short term: cap joint probability at the max of (independent product, max-leg probability) × a correlation penalty when legs share game/team. Long term: see [Make-Sika-Smarter item 8](#make-sika-smarter-consolidated).
