...
def test_lifespan_starts_without_blocking_on_startup_refresh(monkeypatch):
    ...
    monkeypatch.setattr(app_main, "seed_users_from_settings", lambda db, settings: {"inserted": 0, "legacy_ensured": 0})
    monkeypatch.setattr(app_main, "migrate_env_var_credentials_to_owner", lambda db, settings: False)
